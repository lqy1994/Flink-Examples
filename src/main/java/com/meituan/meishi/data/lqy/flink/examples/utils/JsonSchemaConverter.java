...
public final class JsonSchemaConverter {
	...
	private static TypeInformation<?> convertType(String location, JsonNode node, JsonNode root) {
		...
		if (node.has(TYPE)) {
			...
			for (String type : types) {
				...
				switch (type) {
					case TYPE_NULL:
						typeSet.add(BasicTypeInfo.VOID_TYPE_INFO);
						break;
					case TYPE_BOOLEAN:
		...
		} else if (types.size() > 2 || (types.size() == 2 && !types.contains(BasicTypeInfo.VOID_TYPE_INFO))) {
			throw new IllegalArgumentException(
				"Union types with more than just a null type are not supported yet.");
		}

		// return the first non-void type or void
		if (types.size() == 2 && types.get(0) == BasicTypeInfo.VOID_TYPE_INFO) {
			return types.get(1);
		} else {
			return types.get(0);
		}
	...
	private static TypeInformation<Row> convertObject(String location, JsonNode node, JsonNode root) {
		...
		final String[] comment = new String[props.size()];
		...
		while (fieldIter.hasNext()) {
			...
			comment[i]=subNode.getValue().has(DESCRIPTION)?subNode.getValue().get(DESCRIPTION).asText():subNode.getKey();
			// set type
		...
		return new RichRowTypeInfo( types,names,comment);
	}
	...
	private static TypeInformation<?> convertArray(String location, JsonNode node, JsonNode root) {
		...
		if (items.isObject()) {
			...
			return OBJECT_ARRAY(elementType);
		}
	...
	private static TypeInformation<?> convertStringEncoding(String location, JsonNode node) {
		...
		switch (node.asText()) {
			case CONTENT_ENCODING_BASE64:
				return org.apache.flink.table.api.Types.PRIMITIVE_ARRAY(Types.BYTE);
			default:
				// we fail hard here:
